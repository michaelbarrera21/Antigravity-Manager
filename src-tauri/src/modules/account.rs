...
pub fn list_accounts() -> Result<Vec<Account>, String> {
    ...
    if !invalid_ids.is_empty() {
        crate::modules::logger::log_warn(&format!(
            "Found {} invalid account indexes, auto-cleaning...",
            invalid_ids.len()
        ));
...
pub async fn switch_account(account_id: &str) -> Result<(), String> {
    ...
    // 2. 查找账号所属实例（如果没有绑定则使用默认实例）
    let target_instance = match instance::get_instance_for_account(account_id)? {
        Some(inst) => inst,
        None => {
            // 账号未绑定到任何实例，确保默认实例存在并绑定
            crate::modules::logger::log_info(&format!(
                "Account {} not bound to any instance, binding to default instance...",
                account.email
            ));
            let default_inst = instance::ensure_default_instance()?;
            instance::bind_account_to_instance(account_id, &default_inst.id)?;
            default_inst
        }
    };

    crate::modules::logger::log_info(&format!(
        "Target instance: {} (user_data_dir: {:?})",
    ...
    let db_path = db::get_db_path_for_instance(&target_instance.user_data_dir);

    // 确保实例数据目录存在
    device::ensure_instance_data_dir(&target_instance.user_data_dir)?;

    // 5. 只关闭目标实例的进程（不影响其他实例）
    if process::is_instance_running(&target_instance.user_data_dir) {
        crate::modules::logger::log_info(&format!("Closing instance: {}", target_instance.name));
        process::close_instance(&target_instance.user_data_dir, 20)?;
    }

    // 6. Write device profile (generate/bind if missing)
    ...
    if storage_path.exists() {
        ...
        crate::modules::logger::log_warn(&format!(
            "Instance storage.json not found at {:?}, skipping device profile write (will be created on first launch)",
            storage_path
        ));
    }

    // 7. Get database path and backup
    if db_path.exists() {
        ...
        crate::modules::logger::log_warn(&format!(
            "Instance database not found at {:?}, skipping token injection (will be created on first launch)",
            db_path
        ));
    }
    ...
    crate::modules::logger::log_info(&format!(
        "Account switch completed: {} (instance: {})",
        account.email, target_instance.name
    ));
...
/// 在指定实例中切换账号
/// 与 switch_account 类似，但直接使用指定的实例而非查找账号绑定的实例
/// 参数：
/// - account_id: 要切换到的账号 ID
/// - instance: 目标实例
/// - restart_if_running: 如果实例运行中是否重启（true = 重启，false = 只更新配置）
pub async fn switch_account_for_instance(
    account_id: &str,
    ...
    let mut account = load_account(account_id)?;
    crate::modules::logger::log_info(&format!(
        "Switching to account: {} (ID: {}) in instance: {}",
    ...
    let db_path = db::get_db_path_for_instance(&instance.user_data_dir);

    // 确保实例数据目录存在
    device::ensure_instance_data_dir(&instance.user_data_dir)?;

    // 4. 检查实例是否需要重启
    let was_running = if instance.is_default {
        process::is_default_instance_running()
    } else {
        process::is_instance_running(&instance.user_data_dir)
    };
    ...
    if was_running && restart_if_running {
        ...
        if saved_args.is_none() {
            if let Some(args) = process::get_instance_root_process_args(&instance.user_data_dir) {
                // 跳过第一个参数（可执行文件路径）
                saved_args = Some(args.into_iter().skip(1).collect());
            }
        }

        crate::modules::logger::log_info(&format!("Closing instance: {}", instance.name));
        process::close_instance(&instance.user_data_dir, 20)?;
    }

    // 5. Write device profile
    let profile_to_apply = {
        if let Some(p) = account.device_profile.clone() {
            p
        } else {
            crate::modules::logger::log_info(&format!(
                "Account {} has no bound fingerprint, generating new one...",
                account.email
            ));
            let new_profile = device::generate_profile();
            apply_profile_to_account(
    ...
    if storage_path.exists() {
        crate::modules::logger::log_info(&format!(
            "Writing device profile to storage.json for instance: {}",
            instance.name
        ));
        device::write_profile(&storage_path, &profile_to_apply)?;
    } else {
        crate::modules::logger::log_warn(&format!(
            "Instance storage.json not found at {:?}, skipping device profile write",
            storage_path
        ));
    }

    // 6. Inject Token into database
    if db_path.exists() {
        let backup_path = db_path.with_extension("vscdb.backup");
        fs::copy(&db_path, &backup_path)
            .map_err(|e| format!("failed_to_backup_database: {}", e))?;
        ...
        crate::modules::logger::log_info(&format!(
            "Injecting Token into database for instance: {}",
            instance.name
        ));
        db::inject_token(
            &db_path,
        ...
        crate::modules::logger::log_warn(&format!(
            "Instance database not found at {:?}, skipping token injection",
            db_path
        ));
    }

    // 7. Update account last used
    account.update_last_used();
    save_account(&account)?;
    ...
    if was_running && restart_if_running {
        crate::modules::logger::log_info(&format!(
            "Restarting instance: {} with saved args: {:?}",
            instance.name, saved_args
        ));

        if let Some(args) = saved_args {
            if !args.is_empty() {
                process::start_instance_with_args(instance, args)?;
            } else {
                process::start_instance(instance)?;
            }
        } else {
            process::start_instance(instance)?;
        }
    ...
    crate::modules::logger::log_info(&format!(
        "Account switch completed in instance {}: {}",
